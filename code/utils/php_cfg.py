...
class PHP_CFG():
    ...
    def visit_program(self, node):
        ...
        for index, z in enumerate(node.children):
            ...
            if z.type == "compound_statement":  # 526 行
    ...
    def visit_translation_unit(self, node):
        ...
        for index, z in enumerate(node.children):
            ...
            if self.firstlineno > z.start_point[0] + 1:
                self.firstlineno = z.start_point[0] + 1
            if z.type == "function_definition":  # 519 行
                if index == len(node.children) - 1:
                    self.finlineno.append(z.end_point[0] + 1)
                self.ast_visit(z)
            else:
    ...
    def visit_function_definition(self, node):
        for z in node.children:
            if z.type == "compound_statement":  # 526 hang
                self.ast_visit(z)
            else:
                self.visit_piece(z)

    def visit_compound_statement(self, node):  # 复合语句
        # self.G.add_edge(node.start_point[0]+1, node.end_point[0]+1)
        for index, z in enumerate(node.children):
            if z.type == "for_statement":  # 可以使用
                self.ast_visit(z)
            elif z.type == "while_statement":  # 可以使用
                self.ast_visit(z)
            elif z.type == "assert_statement": # 可以使用
                self.ast_visit(z)
            elif z.type == "switch_statement": # 可以使用
                self.ast_visit(z)
            elif z.type == "if_statement":  # 可以使用
                self.ast_visit(z)
            elif z.type == "expression_statement":
                 self.ast_visit(z)
            elif z.type == "continue_statement":  # 可以使用
                self.ast_visit(z)
            elif z.type == "local_variable_declaration":  # 可以使用
                self.ast_visit(z)
            elif z.type == "return_statement":  # 可以使用
                self.ast_visit(z)
            elif z.type == "compound_statement":  # 可以使用
                self.ast_visit(z)
    ...
    def visit_piece(self, node):
        # self.G.add_edge(node.start_point[0]+1, node.end_point[0]+1)
        if node.type == "if_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "while_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "for_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "foreach_statement":  # 新加的
            self.ast_visit(node)
        elif node.type == "switch_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "try_catch_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "throw_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "function_declarator":  # 可以使用
            self.ast_visit(node)
        elif node.type == "class_declaration":  # 可以使用
            self.ast_visit(node)
        elif node.type == "method_declaration":  # 可以使用
            self.ast_visit(node)
        elif node.type == "compound_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "defined_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "namespace_declaration":  # 可以使用
            self.ast_visit(node)
        elif node.type == "continue_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "use_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "echo_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "return_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "include_statement":  # 可以使用
            self.ast_visit(node)
        elif node.type == "function_definition":  # 可以使用
            self.ast_visit(node)
        elif node.type == "pointer_declarator":  # 可以使用
            self.ast_visit(node)
    ...
    def visit_continue_statement(self, node):  # 可以使用
        self.G.add_edge(node.start_point[0], node.start_point[0] + 1, weight=1)
    ...
    def visit_try_statement(self, node):  # 可以使用
        self.G.add_edge(node.start_point[0], node.start_point[0] + 1, weight=1)
        self.dece_node.append(node.start_point[0] + 1)
        body_node = {}
        for z in node.children:
